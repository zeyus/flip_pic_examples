// Modified from Bargteil and Shinar's 2019 SIGGRAPH course code by Chand John
// to use the stb_image_write library to write image files while visualizing a
// particle simulation. FFmpeg can use these files to create a movie of the
// simulation. Particles' color is changed to bluish-turquoise.
...
void saveImage(const std::string& filename) {
  ...
  stbi_write_png(filename.c_str(), width, height, num_channels, buffer.data(), stride);
}
...
void myTimerFunc(int id) {
  if (!singleStep) {
    std::stringstream ss;
    ss << "test_image_";
    if (currentFrame < 10) { ss << "0"; }
    if (currentFrame < 100) { ss << "0"; }
    ss << currentFrame;
    ss << ".png";
